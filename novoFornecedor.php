...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $email = validar($_POST['email'], 'email', true, array('tb_fornecedores' => 'email'));
    if (!$data['validation']['has_error']) {
        $conn = conectar();
        $sql = "INSERT INTO tb_fornecedores(nome, telefone, email, ativo) VALUES(
           '{$data['value']['nome']}',
           '{$data['value']['telefone']}',
           '{$data['value']['email']}',
           1
        ...
        if (mysqli_query($conn, $sql) === true) {
            $data['success'] = "Fornecedor cadastrado com sucesso";
            unset($data['value']);
        } else {
...
<form action="<?=base_url("novoFornecedor.php")?>" method="post">
